...
<div id="con_bottom">
	 <div id="bottom_btn">
		<button type="button" onclick="listPreparing(1)"  class="btn btn-outline-dark">처음</button>
		
		<c:if test="${groupNo>1}">
			<button type="button" onclick="listPreparing(${startPageNo-1})"  class="btn btn-outline-info">이전</button>
		</c:if>
		...
		<div style="display: inline-block;" class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
		  <div class="btn-group mr-2" role="group" aria-label="First group">
		  	<c:forEach begin="${startPageNo}" end="${endPageNo}" var="i">
		  		<c:if test="${pageNo==i}">
		  			<button type="button"  onclick="listPreparing(${i})" class="btn btn-light active">${i}</button>
		  		</c:if>
		  		<c:if test="${pageNo!=i}">
		  		<button type="button"  onclick="listPreparing(${i})" class="btn btn-light">${i}</button>
		  		</c:if>
		  	</c:forEach>
		  </div>
		</div>							
		<c:if test="${groupNo<totalGroupNum}">
			<button type="button"  onclick="listPreparing(${endPageNo+1})" class="btn btn-outline-info">다음</button>
		</c:if>
		<button type="button"  onclick="listPreparing(${totalPageNum})" class="btn btn-outline-dark">맨끝</button>
