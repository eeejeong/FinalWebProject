	...
	private void receiveMessage() throws MqttException {
		client.setCallback(new MqttCallback() {
			...
			public void messageArrived(String topic, MqttMessage message) throws Exception {
				...
				/*else if (mid.equals("MISSION_READ")) {
					logger.debug("===MISSION_READ==");
					String aID = (String) jsonObject.get("aID");
					logger.debug("mission READ 메세지 도착, " + aID);
					Agency agency = missionDao.selectAgencyMission(aID);
					Thread thread = new Thread() {
			            @Override
			            public void run() {
			                	String mission = agency.getMission_waypoint();
			                	System.out.println(mission);
								if(mission == null) mission = "";
			                    JSONObject jsonObject = new JSONObject();
			                    jsonObject.put("msgid", "showMission");
			            		jsonObject.put("agencyId", aID);
			            		jsonObject.put("waypoint",mission);
			            		jsonObject.put("lat", agency.getAgency_latitude());
			            		jsonObject.put("lng",agency.getAgency_longitude());
			            		jsonObject.put("agencyName",agency.getAgency_name());
			                    try {
			                        client.publish("/drone/request/sub", json.getBytes(), 0, false);
